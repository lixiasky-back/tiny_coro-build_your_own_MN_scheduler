...
---
> ⚠️ **Important Notice**
> During the testing of mini-redis, issues including **race condition**, deadlock, and SIGBUS error have been found to **occur alternately** in the code implementation of this repository. **It is not recommended to directly reference or reuse the code implementation temporarily**.
> - The documentation is primarily theoretical and unaffected by the above issues, and can be referenced normally (there may be some oversights in the documentation, which will be optimized synchronously later);
> - The code and relevant documentation will be updated immediately after the fixes for race condition, deadlock, SIGBUS error, and various boundary issues are completed. Please stay tuned.
